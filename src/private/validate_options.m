...
if isfield(options, 'StepTolerance')
    if ~((isrealscalar(options.StepTolerance) && options.StepTolerance > 0) || ...
        ...
        error(sprintf(['options.StepTolerance must be a positive scalar or a positive vector with length ' ...
                    'matching options.num_blocks and not exceeding the problem dimension n.']));
    end
end
...
if isfield(options, 'alpha_init')
    if ~((isrealscalar(options.alpha_init) && options.alpha_init > 0) || ...
        ...
        error('options.alpha_init must be a positive scalar, the string ''auto'', or a positive vector with length matching options.num_blocks and not exceeding the problem dimension n.');
    end
end
