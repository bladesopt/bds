function run_accelerated_bds_dimension_comparison_low()
%RUN_ACCELERATED_BDS_DIMENSION_COMPARISON_LOW Run and redraw the 1-5 tests.

    path_experiment = fileparts(mfilename('fullpath'));
    path_tests = fileparts(path_experiment);
    ...
    raw_savepath = fullfile(path_testdata, ...
        ['accelerated_bds_dimension_comparison_low_raw_1_5_s2mpj_', timestamp]);
    redraw_savepath = fullfile(path_testdata, ...
        ['accelerated_bds_dimension_comparison_low_pairwise_1_5_s2mpj_', timestamp]);
    ...
    pairs = [ ...
        make_pair('E1', 'original_bds_accelerated_bds', [1, 2], ...
            {'Original BDS', 'Accelerated BDS'}), ...
        make_pair('E3', 'accelerated_bds_newuoa', [2, 3], ...
    ...
    fprintf('Accelerated BDS dimension comparison: Low-dimensional raw experiment\n');
    fprintf('Started: %s\n', manifest.started_at);
    ...
    for i_pair = 1:numel(pairs)
        ...
        merged_summary = fullfile(path_summaries, sprintf( ...
            'summary_%s_200n_u_1_5_10features_s2mpj_%s.pdf', pair.slug, timestamp));
...
function [summary_path, solver_scores] = redraw_pairwise_feature(record, pair, pair_savepath)
    ...
    restore_directory = onCleanup(@() cd(previous_directory)); %#ok<NASGU>
