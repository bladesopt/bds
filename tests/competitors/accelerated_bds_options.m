function [xopt, fopt, exitflag, output] = accelerated_bds_options(fun, x0, options)
%ACCELERATED_BDS_OPTIONS solves unconstrained optimization problems without
%using derivatives by blockwise direct search methods with optional
%acceleration steps.
%
%   ACCELERATED_BDS_OPTIONS supports MATLAB R2017b or later.
%
%   XOPT = ACCELERATED_BDS_OPTIONS(FUN, X0) returns an approximate minimizer
%   XOPT of the function FUN, starting the calculations at X0. FUN must
%   accept a vector input X and return a scalar.
%
%   XOPT = ACCELERATED_BDS_OPTIONS(FUN, X0, OPTIONS) performs the
...
%   function value FOPT.
%
%   [XOPT, FOPT, EXITFLAG] = ACCELERATED_BDS_OPTIONS(...) also returns an EXITFLAG that indicates
...
for iter = 1:maxit
    ...
    % accel_state packs exactly the mutable state the phase may update; the
    % terminate/exitflag update derived from the phase result stays here.
    accel_state = struct( ...
    ...
    [accel_state, pre_poll_result] = run_productive_direction_memory_phase( ...
        fun, accel_state, accel_config, mean(alpha_all));
    xbase = accel_state.xbase; fbase = accel_state.fbase;
    nf = accel_state.nf; fhist = accel_state.fhist;
    xhist = accel_state.xhist; invalid_points = accel_state.invalid_points;
    ...
    if ~terminate && iteration_improved && iteration_step_norm > max(alpha_tol) ...
        ...
        accel_state = struct( ...
            ...
            'productive_direction_memory', productive_direction_memory, ...
            'momentum', momentum);
        [accel_state, post_poll_result] = run_post_poll_acceleration_phase( ...
            fun, accel_state, accel_config, iteration_step);
        xbase = accel_state.xbase; fbase = accel_state.fbase;
        nf = accel_state.nf; fhist = accel_state.fhist;
        xhist = accel_state.xhist; invalid_points = accel_state.invalid_points;
        productive_direction_memory = accel_state.productive_direction_memory;
    ...
    if ~post_poll_acceleration_succeeded && all(batch_gradient_available)
        ...
        if (norm(grad) <= sqrt(n)*1e30) && (norm(grad) > 10*sqrt(n)*eps)
            ...
            if use_estimated_gradient_stop || output_gradient_stop_diagnostics
                ...
                if record_gradient_norm
                    reference_grad_norm_after = reference_grad_norm;
                ...
                if output_gradient_stop_diagnostics
                    ...
                    gradient_stop_diagnostics(trace_index) = struct( ...
                        ...
                        'reference_gradient_norm_before', reference_grad_norm_before, ...
                        'reference_gradient_norm_after', reference_grad_norm_after, ...
                        'norm_gradient_window_before', norm_grad_window_before, ...
