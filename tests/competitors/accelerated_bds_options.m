...
% Initialize acceleration state variables.
...
for iter = 1:maxit
    ...
    % accepted update step vector. If norm(block_step) > alpha_tol(i), we store
    % d = block_step / norm(block_step) together with stored_step =
    % norm(block_step).
    %
    ...
    if options.use_productive_direction_memory && ~isempty(prod_memory) && nf < MaxFunctionEvaluations
        ...
        for m_idx = 1:numel(prod_memory)
            ...
            if f_cand < fbase
                ...
                prod_memory(m_idx) = [];
                prod_memory = insert_accelerated_bds_memory_front(prod_memory, dir_vec, step);
                if fbase <= ftarget
                    target_reached = true;
                    terminate = true;
                    exitflag = get_exitflag("FTARGET_REACHED");
    ...
    for i = 1:length(block_indices)
        ...
        % Acceleration bookkeeping: remember the base point before a possible
        % successful polling update in this block.
        ...
        if ~strcmpi(block_visiting_pattern, "parallel") && update_base
            ...
            if options.use_productive_direction_memory && block_step_norm > alpha_tol(i_real)
                prod_memory = remember_accelerated_bds_direction( ...
    ...
    % Acceleration pattern / momentum step after regular polling.
    ...
    %     alpha_pat   = max(norm(iter_step), max(alpha_tol)).
    ...
    if ~terminate && iter_improved && iter_step_norm > max(alpha_tol) && nf < MaxFunctionEvaluations ...
            && (options.use_sweep_pattern_direction || options.use_momentum_extrapolation)
        pattern_dir = iter_step / iter_step_norm;
        alpha_pat = max(iter_step_norm, max(alpha_tol));
