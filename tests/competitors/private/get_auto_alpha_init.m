...
if ~(isnumeric(x0) && isreal(x0) && isvector(x0) && ...
        all(isfinite(x0(:))))
    error('accelerated_bds_options:get_auto_alpha_init:InvalidX0', ...
...
if ~(isnumeric(StepTolerance) && isreal(StepTolerance) && ...
    ...
    error('accelerated_bds_options:get_auto_alpha_init:InvalidStepTolerance', ...
...
elseif numel(step_tolerance) ~= numel(abs_x0)
    error('accelerated_bds_options:get_auto_alpha_init:InvalidStepToleranceLength', ...
...
if any(~isfinite(alpha_coord)) || any(alpha_coord <= 0)
    error('accelerated_bds_options:get_auto_alpha_init:InvalidResult', ...
...
if ~(isnumeric(value) && isreal(value) && isscalar(value) && ...
    ...
    error('accelerated_bds_options:get_auto_alpha_init:InvalidCoefficient', ...
